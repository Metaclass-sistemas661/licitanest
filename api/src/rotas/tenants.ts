...
export async function rotasTenants(app: FastifyInstance) {
  ...
  app.get("/api/tenants/municipios", async (req, reply) => {
    try {
      const { rows } = await getPool().query(
        `SELECT m.*,
                (SELECT c.numero_contrato FROM contratos c WHERE c.municipio_id = m.id AND c.status = 'ativo' LIMIT 1) AS contrato_ativo
         FROM municipios m
         ORDER BY m.nome`,
      );
  ...
  app.get("/api/tenants/estatisticas", { preHandler: [exigirAdmin] }, async (_req, reply) => {
    try {
      const pool = getPool();
      const [municipios, servidores, cestas] = await Promise.all([
